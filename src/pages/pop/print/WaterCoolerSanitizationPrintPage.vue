  ...
  <div class="pop-print" v-if="data">
    <div class="form-header">
      ...
      <div class="form-title-block">
        <div class="form-title">Registro de Higienização do Bebedouro</div>
        <div class="form-subtitle">Frequência: Semanal</div>
      </div>
...
import { getWaterCoolerSanitization } from 'src/services/pop/waterCoolerSanitization.service';
import type { WaterCoolerSanitizationDetail } from 'src/schemas/pop/waterCoolerSanitization.schemas';
import { getPopSignatures } from 'src/services/pop/popSignature.service';

const route = useRoute();
const data = ref<WaterCoolerSanitizationDetail | null>(null);
...
onMounted(async () => {
  ...
  const [detail, sigs] = await Promise.all([
    getWaterCoolerSanitization(id),
    getPopSignatures('water-cooler-sanitization', id),
  ]);
  data.value = detail;
