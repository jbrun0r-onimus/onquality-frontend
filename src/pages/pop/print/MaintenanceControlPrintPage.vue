<template>
  <div class="pop-print" v-if="data">
    ...
    <div class="section-title">CONTROLE DE MANUTENÇÃO</div>
    <table class="data-table">
      <thead>
        <tr>
          <th class="num-col" rowspan="2">Nº da caixa</th>
          <th rowspan="2">Data</th>
          <th colspan="2">Tampa com vedação e ausente de trincas?</th>
          <th colspan="2">Vazamentos/Infiltrações?</th>
          <th colspan="2">Descascamentos?</th>
          <th rowspan="2">Executado por</th>
        </tr>
        <tr>
          <th class="yn-col">SIM</th><th class="yn-col">NÃO</th>
          <th class="yn-col">SIM</th><th class="yn-col">NÃO</th>
          <th class="yn-col">SIM</th><th class="yn-col">NÃO</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          ...
          <td class="text-center">{{ data.question2 === true ? 'X' : '' }}</td>
          <td class="text-center">{{ data.question2 === false ? 'X' : '' }}</td>
          <td class="text-center">{{ data.question3 === true ? 'X' : '' }}</td>
          <td class="text-center">{{ data.question3 === false ? 'X' : '' }}</td>
          <td>{{ data.employee_name }}</td>
        </tr>
        <tr v-for="n in 4" :key="n"><td>&nbsp;</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
      </tbody>
    </table>
...
onMounted(async () => {
  ...
  const [detail, sigs] = await Promise.all([
    getMaintenanceControl(id),
    getPopSignatures('maintenance-control', id),
  ]);
  data.value = detail;
