    ...
    <q-form @submit="onSubmit" greedy>
      ...
      <q-card-actions align="right">
        ...
        <app-button
          ...
          @click="openPrintWindow('sanitation-control', props.item.id)"
        />
        <app-button
          v-if="!readonly"
          type="submit"
...
import {
  sanitationControlFormSchema,
  SanitationControlForm,
  SanitationControlListItem,
} from 'src/schemas/pop/sanitationControl.schemas';
import {
  createSanitationControl,
  updateSanitationControl,
  sanitationControlDetailQuery,
...
const title = computed(() => {
  if (props.readonly) return t('pop.sanitationControl.titles.details');
  return props.item ? t('pop.sanitationControl.titles.edit') : t('pop.sanitationControl.titles.new');
});

const questions = [
  { key: 'question1', label: t('pop.sanitationControl.questions.q1') },
  { key: 'question2', label: t('pop.sanitationControl.questions.q2') },
  { key: 'question3', label: t('pop.sanitationControl.questions.q3') },
  { key: 'question4', label: t('pop.sanitationControl.questions.q4') },
  { key: 'question5', label: t('pop.sanitationControl.questions.q5') },
  { key: 'question6', label: t('pop.sanitationControl.questions.q6') },
  { key: 'question7', label: t('pop.sanitationControl.questions.q7') },
  { key: 'question8', label: t('pop.sanitationControl.questions.q8') },
  { key: 'question9', label: t('pop.sanitationControl.questions.q9') },
  { key: 'question10', label: t('pop.sanitationControl.questions.q10') },
  { key: 'question11', label: t('pop.sanitationControl.questions.q11') },
  { key: 'question12', label: t('pop.sanitationControl.questions.q12') },
  { key: 'question13', label: t('pop.sanitationControl.questions.q13') },
  { key: 'question14', label: t('pop.sanitationControl.questions.q14') },
  { key: 'question15', label: t('pop.sanitationControl.questions.q15') },
  { key: 'question16', label: t('pop.sanitationControl.questions.q16') },
  { key: 'question17', label: t('pop.sanitationControl.questions.q17') },
];

const { mutate, isLoading: isLoadingMutation } = useMutation(
  (data: SanitationControlForm) => {
    if (props.item) return updateSanitationControl(props.item.id, data);
    return createSanitationControl(data);
  },
  {
    onSuccess: () => {
      onDialogOK();
      showSuccessFeedback(
        props.item
          ? t('pop.sanitationControl.feedback.updated')
          : t('pop.sanitationControl.feedback.created')
      );
      queryClient.invalidateQueries([sanitationControlsQuery.name]);
      if (props.item)
        queryClient.invalidateQueries([sanitationControlDetailQuery.name, props.item.id]);
    },
  }
);

const { fillRefetch, isFillLoading, fillError } = useFillFormQuery({
  query: sanitationControlDetailQuery,
...
watch(
  () => props.item,
  (item) => { if (item) setValues(item as Partial<SanitationControlForm>, false); },
  { immediate: true }
);
